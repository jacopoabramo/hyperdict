...
# Copyright [2025] [Jacopo Abramo] [jacopo.abramo@gmail.com]
#
...
class SharedLock:
    ...
    def __init__(self, parent: Any, lock_name: str, pid_name: str) -> None:
        self.has_lock = 0
        ...
        try:
            self.ctx = atomics.atomicview(
                buffer=self.lock_remote[0:1], atype=atomics.BYTES
            )
            self.pid_remote_ctx = atomics.atomicview(
                buffer=self.pid_remote[0:4], atype=atomics.BYTES
            )
    ...
    def acquire_with_timeout(
        ...
        while True:
            ...
            except errors.CannotAcquireLock as e:
                if not time_start:
    ...
    def acquire(
        ...
        if timeout:
            return self.acquire_with_timeout(
        ...
        while True:
            ...
            if not block:
                raise errors.CannotAcquireLock(blocking_pid=self.get_remote_pid())

    def test_and_inc(self) -> bool:
    ...
    def release(self, *args: Any) -> bool:
        if self.has_lock > 0:
            ...
            if owner != self.pid:
                raise Exception(
                    f"Our lock for pid {self.pid} was stolen by pid {owner}"
                )
            self.has_lock -= 1
    ...
    def reset_acquire_parameters(self) -> None:
        self.next_acquire_parameters = ()

    def steal(self, from_pid: int = 0, release: bool = False) -> bool:
        if self.has_lock:
            raise Exception(
        ...
        result = self.pid_remote_atomic.cmpxchg_strong(
            expected=from_pid.to_bytes(4, "little"), desired=self.pid_bytes
        )
        if result.success:
            self.has_lock = 1
            if release:
    ...
    def steal_from_dead(self, from_pid: int = 0, release: bool = False) -> bool:
        ...
        try:
            ...
            if (
                p
                and p.is_running()
                and p.status() not in [psutil.STATUS_ZOMBIE, psutil.STATUS_DEAD]
            ):
                raise Exception(
    ...
    def print_status(self, status: Optional[dict[str, Any]] = None) -> None:
        import pprint
    ...
    def get_remote_pid(self) -> int:
        return int.from_bytes(self.pid_remote, "little")

    def get_remote_lock(self) -> int:
    ...
    def __enter__(self) -> "HyperDict.SharedLock":
...
class HyperDict(collections.UserDict, dict):
    ...
    def __init__(
        ...
        *args: Any,
        ...
        full_dump_size: int | None = None,
        auto_unlink: bool = False,
        recurse: bool = False,
        recurse_register: str | HyperDict | None = None,
        **kwargs: Any,
    ) -> None:
        ...
        def finalize(weak_self: weakref.ReferenceType[HyperDict]) -> None:
        ...
        self.buffer = self.get_memory(
            create=create, name=self.name + "_memory", size=buffer_size
        )
        ...
        if hasattr(self.control, "created_by_ultra"):
            ...
            if full_dump_size:
                ...
                self.full_dump_static_size_remote[:] = full_dump_size.to_bytes(
                    4, "little"
                )

                self.full_dump_memory = self.get_memory(
                    create=True, name=self.name + "_full", size=full_dump_size
                )
                self.full_dump_memory_name_remote[:] = (
                    self.full_dump_memory.name.encode("utf-8").ljust(255)
                )
        ...
        # We just attached to the existing control
            ...
            if shared_lock is None:
                ...
                raise errors.ParameterMismatch(
            ...
            if size > 0:
                ...
                self.full_dump_memory = self.get_memory(
                    create=False, name=self.name + "_full"
                )
        ...
        # Parameters that could be read from remote if we are connecting to an existing HyperDict
        self.recurse = recurse

        # In recurse mode, we must ensure a recurse register
        if self.recurse:
            # Must be either the name of an HyperDict as a string or an HyperDict instance
            if recurse_register is not None:
                if isinstance(recurse_register, str):
                    self.recurse_register = HyperDict(name=recurse_register)
                elif isinstance(recurse_register, HyperDict):
            ...
            # If no register was defined, we should create one
                ...
                self.recurse_register = HyperDict(
    ...
    def get_memory(
        *, create: bool = True, name: str, size: int = 0
        ...
        raise errors.CannotAttachSharedMemory(f"Could not get memory '{name}'")

    # @profile
    def dump(self) -> multiprocessing.shared_memory.SharedMemory:
        """Dump the full dict into shared memory"""

        with self.lock:
            old = (
                bytes(self.full_dump_memory_name_remote)
                .decode("utf-8")
                .strip()
                .strip("\x00")
            )
            ...
            if length + 6 > full_dump_memory.size:
                raise errors.FullDumpMemoryFull(
            ...
            if not (self.full_dump_size and self.full_dump_memory):
                self.full_dump_memory_name_remote[:] = full_dump_memory.name.encode(
                    "utf-8"
                ).ljust(255)
    ...
    def get_full_dump_memory(
        self, max_retry: int = 3, retry: int = 0
    ) -> multiprocessing.shared_memory.SharedMemory:
        """
        ...
        except errors.CannotAttachSharedMemory as e:
            if retry < max_retry:
                ...
                with self.lock:
                    return self.get_full_dump_memory(
                        max_retry=max_retry, retry=retry + 1
                    )
            else:
                raise e
        ...
        try:
            if force or (self.full_dump_counter < full_dump_counter):
                ...
                assert length > 0, (
                    self.status(),
                    full_dump_memory,
                    bytes(buf[:]).decode("utf-8").strip().strip("\x00"),
                    len(buf),
                )
                pos += 4
            ...
            full_dump_delta = (
                int.from_bytes(self.full_dump_counter_remote, "little")
    ...
    # @profile
    def append_update(self, key: Any, item: Any, delete: bool = False) -> None:
        ...
        with self.lock:
            start_position = int.from_bytes(
                self.update_stream_position_remote, "little"
            )
    ...
    def apply_update(self) -> None:
        ...
        if self.full_dump_counter < int.from_bytes(
            self.full_dump_counter_remote, "little"
        ):
            self.load(force=True)

        if self.update_stream_position < int.from_bytes(
            self.update_stream_position_remote, "little"
        ):
            ...
            try:
                ...
                while pos < int.from_bytes(
                    ...
                    length = int.from_bytes(
                        bytes(self.buffer.buf[pos : pos + 4]), "little"
                    )
                    pos += 4
    ...
    def update(
        self,
        other: Optional[Union[Mapping[Any, Any], Iterator[tuple[Any, Any]]]] = None,
        *args: Any,
        **kwargs: Any,
    ) -> None:
    ...
    def __setitem__(self, key: Any, item: Any) -> None:
        ...
        with self.lock:
            ...
            if self.recurse:
                assert isinstance(self.recurse_register, HyperDict), (
                    "recurse_register must be an HyperDict instance"
                )
    ...
    def status(self) -> dict[str, Any]:
        ...
        ret = {
            ...
            if hasattr(self, attr) and attr != "data"
        }

        ret["update_stream_position_remote"] = int.from_bytes(
            self.update_stream_position_remote, "little"
        )
        ...
        ret["lock"] = self.lock
        ret["full_dump_counter_remote"] = int.from_bytes(
            self.full_dump_counter_remote, "little"
        )
        ret["full_dump_memory_name_remote"] = (
            bytes(self.full_dump_memory_name_remote)
            .decode("utf-8")
            .strip("\x00")
            .strip()
    ...
    def print_status(
        self, status: Optional[dict[str, Any]] = None, stderr: bool = False
    ) -> None:
    ...
    def raise_already_closed(self, *args: Any, **kwargs: Any) -> None:
        raise errors.AlreadyClosed(
            "HyperDict already closed, you can only access the `HyperDict.data` buffer!"
        )
    ...
    def close(
        self, unlink: bool = False, from_finalizer: bool = False
    ) -> dict[Any, Any]:
    ...
    def unlink_recursed(self) -> None:
        # log.debug("Unlink recursed id={}", hex(id(self)))
        if not self.recurse or (not isinstance(self.recurse_register, HyperDict)):
            raise Exception("Cannot unlink recursed for non-recurse HyperDict")
    ...
    @staticmethod
    def unlink_by_name(name: str, ignore_errors: bool = False) -> bool:
        """
        Can be used to delete left over shared memory blocks after crashes.
        """
        try:
            # log.debug("Unlinking memory '{}'", name)
            memory = HyperDict.get_memory(create=False, name=name)
            memory.unlink()
            memory.close()
            return True
        except errors.CannotAttachSharedMemory as e:
